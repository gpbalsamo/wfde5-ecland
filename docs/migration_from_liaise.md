...
| File | Lines | LIAISE hits | Class | Note |
|---|---:|---:|---|---|
| `init_clim.py` | 894 | **0** | **A** | Zero LIAISE-specific references. Already grid-shape-agnostic: `gen_file` (reduced-Gaussian/point representation) vs. `gen_file_LL` (regular lat/lon) are both implemented, `get_mask`/`cut_mask` take a `BBOX` parameter rather than a hardcoded one, `grib2nc`/`grib2nc_LL` take explicit `nlat`/`nlon`. This is the strongest reuse candidate in the whole repo and should be ported first, unmodified except for import paths. |
